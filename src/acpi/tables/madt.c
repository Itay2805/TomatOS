...
error_t madt_init() {
    ...
    log_info("\t\tOEM ID: %.6s", madt->header.oemid);
    log_info("\t\tOEM TABLE ID: %.8s", madt->header.oem_table_id);
    ...
    for(uintptr_t addr = (uintptr_t) madt->entries; addr < (uintptr_t)madt->entries + (madt->header.length - sizeof(madt_t)); addr += cur->length, cur = (madt_entry_t *) addr) {
        switch(cur->type) {
            case MADT_TYPE_LAPIC: {
                ...
                log_info("\t\t\tLocal APIC #%d (0x%016p)", lapic->id, (uintptr_t)lapic - DIRECT_MAPPING_BASE);
                log_info("\t\t\t\tProcessor ID: %d (%s)", lapic->processor_id, lapic->processor_enabled ? "enabled" : "disabled");
                arrpush(madt_lapics, lapic);
            } break;
            case MADT_TYPE_IOAPIC: {
                madt_ioapic_t* ioapic = &cur->ioapic;
                log_info("\t\t\tI/O APIC #%d (0x%016p)", ioapic->id, (uintptr_t)ioapic - DIRECT_MAPPING_BASE);
                log_info("\t\t\t\tMMIO: 0x%016x", ioapic->mmio_base);
                log_info("\t\t\t\tGSI base: 0x%08x", ioapic->gsi_base);
                arrpush(madt_ioapics, ioapic);
            } break;
            case MADT_TYPE_ISO: {
                madt_iso_t* iso = &cur->iso;
                log_info("\t\t\tISO #%d (0x%016p) %d->%d:%d", arrlen(madt_isos), (uintptr_t)iso - DIRECT_MAPPING_BASE, iso->gsi, iso->bus_source, iso->irq_source);
                // TODO: Flags
                arrpush(madt_isos, iso);
            } break;
            case MADT_TYPE_NMI: {
                madt_nmi_t* nmi = &cur->nmi;
                log_info("\t\t\tNMI #%d (0x%016p)", arrlen(madt_nmis), (uintptr_t)nmi - DIRECT_MAPPING_BASE);
                if(nmi->processor_id == 0xFF) {
                    log_info("\t\t\t\tProcessor: all");
                }else {
                    log_info("\t\t\t\tProcessor: %d", nmi->processor_id);
                }
                log_info("\t\t\t\tLINT: %d", nmi->lint);
                // TODO: Flags
