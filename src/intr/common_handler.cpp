...
namespace intr {
    ...
    static void default_interrupt_handler(interrupt_context* regs) {
        ...
        if(name == nullptr) {
            util::log() << "[!] Unhandled interrupt: " << regs->int_num << frg::endlog;
            return;
        }else {
            util::log() << "[-] Exception occurred: " << name << frg::endlog;
        }
        ...
        switch(regs->int_num) {
            ...
            case 13: // General Protection Fault
                if(regs->error_code != 0) {
                    util::log() << "[-] Selector(processor=" << PROCESSOR_NAME[regs->error_code & 0b1u] << ", table=" << TABLE_NAME[(regs->error_code >> 1u) & 0b11u] << ", index=" << (int) (regs->error_code & 0xFFF8u) << ")" << frg::endlog;
                }
                break;
            case 14: // page fault
                util::log() << "[-] Reason: " << PRESENT_NAMES[regs->error_code & 1u] << frg::endlog;
                util::log() << "[-] Address: 0x" << frg::hex_fmt(arch::read_cr2()) << frg::endlog;
                util::log() << "[-] Mode: " << USER_NAME[(regs->error_code >> 2u) & 1u] << frg::endlog;
                if(((regs->error_code >> 4u) & 1u) != 0) {
                    util::log() << "[-] Operation: Instruction Fetch" << frg::endlog;
                }else if(((regs->error_code >> 3u) & 1u) != 0) {
                    util::log() << "[-] Operation: Reserved write" << frg::endlog;
                }else {
                    util::log() << "[-] Operation: " << OPERATION_NAME[(regs->error_code >> 1u) & 1u] << frg::endlog;
                }
                break;
            default:
                break;
