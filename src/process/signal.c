...
error_t signal_route(signal_handler_t* handler) {
    ...
    switch(handler->type) {
        ...
        case SIGNAL_CALLBACK: {
            ...
            stack[-1] = thread->state.cpu.rax; thread->state.cpu.rsp -= 8;
            stack[-2] = thread->state.cpu.rdi; thread->state.cpu.rsp -= 8;
            stack[-3] = thread->state.cpu.rsi; thread->state.cpu.rsp -= 8;
            stack[-4] = thread->state.cpu.rdx; thread->state.cpu.rsp -= 8;
            stack[-5] = thread->state.cpu.rcx; thread->state.cpu.rsp -= 8;
            stack[-6] = thread->state.cpu.r8; thread->state.cpu.rsp -= 8;
            stack[-7] = thread->state.cpu.r9; thread->state.cpu.rsp -= 8;
            stack[-8] = thread->state.cpu.r10; thread->state.cpu.rsp -= 8;
            stack[-9] = thread->state.cpu.r11; thread->state.cpu.rsp -= 8;

            // we want to return to the restoration stub
            stack[-10] = (uint64_t) signal_return; thread->state.cpu.rsp -= 8;
