...
// changed so it will work on a single tlsf since we don't care about it
...
/* we only have a single static one */
...
** sentinel block.
*/
static size_t tlsf_pool_overhead(void)
{
    return 2 * block_header_overhead;
}

static pool_t tlsf_add_pool(void* mem, size_t bytes)
{
    ...
    {
        TRACE("tlsf_add_pool: Memory must be aligned by %u bytes.\n",
    ...
    {
        TRACE("tlsf_add_pool: Memory size must be between 0x%x and 0x%x00 bytes.\n",
...
//static void tlsf_remove_pool(pool_t pool)
//{
...
** TLSF main interface.
*/
static void tlsf_create()
{
    control_construct();
}

static void* tlsf_malloc(size_t size)
{
...
//static void* tlsf_memalign(size_t align, size_t size)
//{
//    const size_t adjust = adjust_request_size(size, ALIGN_SIZE);
//
//    /*
...
static void tlsf_free(void* ptr)
{
...
**   contents undefined
*/
static void* tlsf_realloc(void* ptr, size_t size)
{
    void* p = 0;
...
void* mm_allocate(size_t size) {
    void* alloc = tlsf_malloc(size);

    // didn't have enough memory, add a pool
    if (alloc == NULL) {
        size_t pgcount = SIZE_TO_PAGES(size);
        uintptr_t base = 0;
        pmm_allocate_pages(ALLOCATE_ANY, pgcount, &base);
        base = PHYSICAL_TO_DIRECT(base);
        tlsf_add_pool((void*)base, PAGES_TO_SIZE(size));
...
void* mm_reallocate(void *ptr, size_t size) {
    void* alloc = tlsf_realloc(ptr, size);
