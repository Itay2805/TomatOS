...
static void set_mmio_base(pcidev_t* dev) {
    dev->mmio_base = (char*)CONVERT_TO_DIRECT(current->base + (((dev->bus - current->start_pci_bus) << 20) | (dev->device << 15) | dev->function << 12));
    if(!vmm_is_mapped(kernel_address_space, (uintptr_t) dev->mmio_base)) {
        vmm_map(kernel_address_space, dev->mmio_base, dev->mmio_base - DIRECT_MAPPING_BASE, PAGE_ATTR_WRITE);
    }
}
...
static void check_function(pcidev_t* dev) {
    ...
    if(pci_get_vendor_name(dev) == NULL) {
        log_info("\t%x.%x.%x.%x (0x%016p) -> %s", dev->segment, dev->bus, dev->device, dev->function, dev->mmio_base - DIRECT_MAPPING_BASE, pci_get_name(dev));
    }else {
        log_info("\t%x.%x.%x.%x (0x%016p) -> %s %s", dev->segment, dev->bus, dev->device, dev->function, dev->mmio_base - DIRECT_MAPPING_BASE, pci_get_vendor_name(dev), pci_get_name(dev));
    }
    arrpush(pcidevs, *dev);
}
